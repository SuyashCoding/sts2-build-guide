...
import { getEntityType, getCardImageUrl, EntityType } from '@/lib/game-entities'
...
> = {
  card:   { text: 'text-entity-card',   bg: 'bg-entity-card/15',   border: 'border-entity-card/50',   label: 'Card'   },
  relic:  { text: 'text-entity-relic',  bg: 'bg-entity-relic/15',  border: 'border-entity-relic/50',  label: 'Relic'  },
  potion: { text: 'text-entity-potion', bg: 'bg-entity-potion/15', border: 'border-entity-potion/50', label: 'Potion' },
  enemy:  { text: 'text-entity-enemy',  bg: 'bg-entity-enemy/15',  border: 'border-entity-enemy/50',  label: 'Enemy'  },
}
...
function EntityTooltip({
  ...
  return (
    ...
    >
      ...
      <span className={cn('text-xs font-bold leading-tight', style.text)}>{name}</span>
      <span
        className={cn(
  ...
  return (
    <>
      <span
        ...
        className={cn(
          'font-bold cursor-pointer transition-colors underline underline-offset-2 decoration-dotted',
...
export function BuildsDocPanel({ character, colors }: BuildsDocPanelProps) {
  ...
  const strongRenderer = useCallback(
    ({ children }: { children?: React.ReactNode }) => {
      ...
      if (!entityType) {
        return <strong className={cn('font-bold', colors.text)}>{children}</strong>
      }
      return (
      ...
      <div className="flex min-h-[480px]">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-4 px-5 py-1.5 border-b border-border/40 flex-wrap">
            <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wider mr-1">Legend</span>
            {(Object.entries(ENTITY_STYLE) as [EntityType, typeof ENTITY_STYLE[EntityType]][]).map(([type, style]) => (
              <span key={type} className={cn('text-[11px] font-semibold flex items-center gap-1', style.text)}>
                <span className={cn('inline-block w-2 h-2 rounded-full', style.bg)} style={{ border: `1.5px solid currentColor` }} />
                {style.label}
              </span>
            ))}
          </div>
